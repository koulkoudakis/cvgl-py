...
cv = cvGameDevTools.CvTool(300, 300, mat_size=(32, 32))
...
while run:
    ...
    if xp != None and yp != None:
        ...
        if index_finger:
            if game_over == True:
                if again_rect.collidepoint((xp, yp)):
                    # reset variables
                    game_over = False
                    player = 1
                    pos = (0, 0)
    ...
    # frame, overlayed, matimg = cv.frame_step()

    pygameoverlayed = cv2.addWeighted(frame_img, 0.3, overlayed, 0.7, 0)
    cv2.imshow('game overlayed', cv2.resize(pygameoverlayed, (600, 600)))
